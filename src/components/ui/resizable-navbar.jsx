...
import { motion, AnimatePresence, useScroll, useMotionValueEvent } from "framer-motion"; // Updated

import { cn } from "../../utils/cn"; // Or use 'clsx' if needed
    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          key={`link-${idx}`}
          to={item.link} // Changed from href
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
  ...
  return (
    ...
    >
      {/* <img
        ...
        height={30}
      /> */}
      <span className="font-semibold text-black text-xl dark:text-white">ScreenCast</span>
    </Link>
  );
};
